...
def custom_collate(batch):
    ...
    for l in labels:
        ...
        cleaned_labels.append(torch.tensor(lbls))  
    labels = torch.stack(cleaned_labels)

    return images, texts, labels, names
    ...
    def __init__(
        ...
        *,
        ...
        self.valid_ds = CTReportDatasetinfer(
            data_folder=data_valid, csv_file=reports_file_valid, labels=labels, split='test'
        )
        ...
        self.dl_iter = iter(cycle(self.dl))
        self.valid_dl_iter = iter(cycle(self.valid_dl))
            ...
            with torch.no_grad():
                ...
                for model, filename in models_to_evaluate:
                    ...
                    for i in range(len(self.valid_ds)):
                        ...
                        print(f"Loaded validation data batch {i}")
