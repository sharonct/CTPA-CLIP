...
class CTViT(nn.Module):
        ...
        *,
        ...
        if use_vgg_and_gan:
            ...
            self.discr = nn.Sequential(
                nn.Conv2d(channels, discr_base_dim, 4, stride=2, padding=1),
                leaky_relu(),
                nn.Conv2d(discr_base_dim, discr_base_dim * 2, 4, stride=2, padding=1),
                nn.BatchNorm2d(discr_base_dim * 2),
                leaky_relu(),
                nn.Conv2d(discr_base_dim * 2, discr_base_dim * 4, 4, stride=2, padding=1),
                nn.BatchNorm2d(discr_base_dim * 4),
                leaky_relu(),
                nn.Conv2d(discr_base_dim * 4, discr_base_dim * 8, 4, stride=2, padding=1),
                nn.BatchNorm2d(discr_base_dim * 8),
                leaky_relu(),
                nn.Conv2d(discr_base_dim * 8, 1, 4, stride=1, padding=0)
            )
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        transform = T.Compose([T.Resize(256)])
        input_vgg_input2 = transform(input_vgg_input2).to(device)
        recon_vgg_input2 = transform(recon_vgg_input2).to(device)
